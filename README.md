...
# 今後の課題
## 1. テスト方法が十分でない
- 初めてテストツールを使ったため，テストが十分に行えているかわからない
- テストのやり方をもっと学んで，効率的かつ効果的に行いたい

## 2. APIの取得に時間がかかってしまう
- 一回のAPI呼び出しではそこまでかからないが，全都道府県のデータを取得するときに非常に時間がかかってしまう
- fetchを行う際に，Promis.all()を用いて並列で行っているが，あまり速度が上昇できなかった
- どのようにしたらもっとAPI呼び出しが早くなるかを学んで，即時応答性を高めたい
